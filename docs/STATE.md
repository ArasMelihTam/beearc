...
**M5a + M5b + M5c + M5d + M5e: ✅ code complete (2026-08-12), committed as `ed82e85`, device acceptance PENDING — first launch attempt on 2026-08-13 hit a migration-ledger crash, now fixed (see "Migration ledger repair" below); the block still has to be tested on a phone.** M5 was split into M5a (queens, treatments, R2 + R4 wiring) and M5b (equipment, transfers, R1 wiring). M5c, M5d and M5e are user-requested blocks: M5c and M5d landed before M5b (inspection viewing/editing/deleting, elapsed-time wording, last-inspection recency on the hive list), M5e after it (sliding summary lines, "other harmful insects seen"). A hive condition score was built in M5c and **removed again on 2026-08-12** at the user's request (see its section below). Next: user tests all five blocks on device, then M6 (photos & voice).

Verified off-device at the end of this session: `tsc --noEmit` clean, **62/62 Jest green**, en/tr key parity **339/339**, no missing i18n keys, every enum value translated in both languages, eslint unchanged (3 pre-existing issues), migration chain 0000→0005 applied in real SQLite with FK enforcement, **SCHEMA_VERSION 6**.
...
### FIXED 2026-08-13 — "Something went wrong opening the database" (migration 0005 re-ran)
